...
// GLOBALS
static char fVersion[4] = {1, 0, 5, 4}; // FIRMWARE VERSION, [ <major_1>, <major_2>, <minor>, <patch>]
static uint16_t RTC;
...
void main(void) {
    ...
    // ADC TIMING
    // 32 us Tad is not recommended in datasheet p. 510-511
    // Too small Tad reduces accuracy due to too fast ADC's internal operation. Keep it > 1.6 us.
    // Acquisition time is controlled by ADACQL.
    ADCON0bits.CS = 0;      // ADC clock supplied by Fosc
    ADCLK = 0x0A;           // Tad = (2*(ADCLK[5:0]+1)) / Fosc , 0x0A --> 5.5 us Tad
    ADACQH = 0x0;           // Acquisition delay high byte (upper 8 bits) not used, set to 0
    ADACQL = 0x0F;          // Acquisition delay low byte, 0x0F --> 15 x 0.28 us = 4.2 us
